...
def os_discrete_balanced(os_file_path, num_bins=3, embedding_dir=None):
    ...
    os_df = os_df[["Patient card::Patient cohort code_Patient Card", "OS_KaplanM_allHGSC", "Survival"]]
    os_df.columns = ["patient_id", "OS", "Survival"]
    ...
    if embedding_dir is not None:
        ...
        print(f"🎯 Pazienti filtrati con embedding disponibili: {len(os_df)}")

    # Crea bin con distribuzione uniforme
    os_df["OS_bin"], bin_edges = pd.qcut(os_df["OS"], q=num_bins, retbins=True, labels=range(num_bins))
    print(f"📊 Confini dei bin di OS: {bin_edges}")
    print(f"📈 Conteggio per ciascun bin:\n{os_df['OS_bin'].value_counts().sort_index()}")
...
def os_binary(os_file_path, embedding_dir=None):
    ...
    # Carica i dati di OS
    os_df = pd.read_excel(os_file_path)
    os_df = os_df[["Patient card::Patient cohort code_Patient Card", "OS_KaplanM_allHGSC"]]
    os_df.columns = ["patient_id", "OS"]
    ...
    threshold = 874.50
    print(f"Soglia OS (mediana): {threshold:.2f}")
    ...
    print(f"📈 Conteggio per ciascuna label:\n{label_counts}")
...
def pfi_discrete(pfi_file_path, embedding_dir):
    ...
    pfi_df.columns = ["patient_id", "PFI", "Survival"]
    ...
    if removed > 0:
        print(f"⚠️ Rimossi {removed} pazienti con PFI mancante")
    ...
    print(f"🔎 Rimossi {filtered_out} pazienti senza embedding. Rimasti: {len(pfi_df)}")
...
def hr_binary(labels_path_hr, embedding_dir):
    """
    Restituisce un dizionario {patient_id: hr_binary} per i pazienti PDS
    con almeno 2 su 3 score HR concordanti (escludendo 'conflicting'),
    e per cui esistono embeddings.
    ...
    df = pd.read_csv(labels_path_hr, sep='\t')
    df_pds = df[df['Treatment strategy'] == 'PDS'].copy()
    ...
    # === Determina original_hr (2 su 3 concordi) ===
