...
class GAT_with_GAP_norm_leakyRELU_06_GraphNorm(torch.nn.Module):
    def __init__(self, input_dim=768, embedding_size=64, num_heads=4, dropout_rate=0.6, output_size=1):
        ...
        # GAT layers
        self.initial_conv = GATConv(input_dim, embedding_size, heads=num_heads, concat=True)  # 768 → 64 × 4 = 256
        self.graph_norm = GraphNorm(embedding_size * num_heads)  # GraphNorm after first GATConv

        self.conv1 = GATConv(embedding_size * num_heads, embedding_size, heads=num_heads, concat=True)  # 256 → 64 × 4 = 256
        self.graph_norm1 = GraphNorm(embedding_size * num_heads)  # GraphNorm after second GATConv
        
        # Pooling Attentivo Globale
        self.attention_pool = GlobalAttention(nn.Linear(embedding_size * num_heads, 1))  # Pooling

        # Output layer
        self.out = nn.Linear(embedding_size * num_heads, output_size)  # 256 → 5
